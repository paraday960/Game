...
func _ready():
	...
	if AIAdvisor.agents.size() != 69 or diagnoses.size() != 69:
		failed.append("شورای هوشمند همه ۶۹ سامانه را تحلیل نکرد (شامل هوش رهبر و رقبای داخلی — عمق‌بخشی ۴۸/۴۹)")
	else:
		...
		if advisor_cmds.is_empty():
			...
			if not advisor_result.success:
				...
				print("AI advisor: 69 diagnoses + valid action OK")
	...
	if not probe1.success:
		...
		if broken_profiles.size() > 0:
			...
			print("AI diagnostic profiles: همه ۶۹ مسیر شاخص عددی قابل خواندن است: OK")
