...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# واحد cadence (دور دوازدهم): investment_growth نرخ سالانه است؛ هفتگی ⇒ ×۶/۳۶۵.
	# پیش از این انباشت سرمایهٔ خصوصی ~۱٪/سال می‌چرخید (یک‌ششم طراحی) و در برابر رشد
	# GDP کوچک می‌ماند.
	priv["investment"] *= (1.0 + priv["investment_growth"] * 6.0 / 365.0)
	...
	# ── لایه واقع‌گرایانه اختصاصی بخش خصوصی (جایگزین قالب خودکار تکراری) — بخش ۳.۵۸ ──
	# فضای کسب‌وکار: قاعده قانون سازنده؛ فساد و نرخ بهره بالا خفه‌کننده
	var rule_ps: float = float(state.get("judicial", {}).get("rule_of_law", 0.60))
	var corr_ps: float = float(state.get("politics", {}).get("corruption", 0.30))
	var rate_ps: float = float(state.get("central_bank", {}).get("interest_rate", 0.15))
	priv["business_climate"] = clampf(float(priv.get("business_climate", 0.60)) * 0.997 + (rule_ps * 0.4 + (1.0 - corr_ps) * 0.4 + (1.0 - rate_ps * 2.0) * 0.2) * 0.003, 0.10, 0.95)
	# در فضای کسب‌وکار خوب کسب‌وکارهای نوپا بیشتر رویش دارند
	priv["startups"] = maxf(float(priv.get("startups", 5000.0)) * (1.0 + (float(priv.get("business_climate", 0.60)) - 0.55) * 0.0006), 100.0)
	if float(priv.get("business_climate", 0.60)) < 0.30 and Deterministic.chance(0.004):
		events.append({"type": "business_exodus", "message": "خروج بنگاه‌های خصوصی - فضای کسب‌وکار خفه‌کننده است", "climate": priv["business_climate"]})
	state["private_sector"] = priv
