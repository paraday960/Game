...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	var _sys_extra = state.get("industry", {}) if state.has("industry") else sys if 'sys' in locals() else {}
	...
	var _productivity = float(state.get("industry",{}).get("productivity",0.60) if state.has("industry") else 0.60)
	_productivity = clamp(_productivity*0.98 + _happiness*0.01 + _growth*5.0*0.01 + _infra_q*0.01, 0.10, 0.95)
	if state.has("industry") and state["industry"] is Dictionary:
		state["industry"]["productivity"] = _productivity
