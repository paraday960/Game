...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# وقت‌شناسی از سن ناوگان و ترافیک واقعی جاده‌ها (پیوند سیستم راه‌ها) — ناوگان پیر و ترافیک سنگین = تأخیر
	var congestion_pt = float(state.get("transport_detail", {}).get("traffic_congestion", 0.40))
	var punct_target = 0.85 - float(pt.get("fleet_age", 7.0)) * 0.02 - congestion_pt * 0.25
	pt["punctuality"] = clampf(float(pt.get("punctuality", 0.75)) * 0.99 + punct_target * 0.01, 0.15, 0.98)
	# ایمنی: ناوگان پیر تصادف بیشتر
	pt["accident_rate"] = clampf(0.008 + float(pt.get("fleet_age", 7.0)) * 0.002 - float(pt.get("electrification", 0.15)) * 0.005, 0.003, 0.08)
	# رضایت از سه پایه: وقت‌شناسی، مقرون‌به‌صرفگی، پوشش
	var sat_target = float(pt.get("punctuality", 0.75)) * 0.4 + float(pt.get("affordability", 0.70)) * 0.3 + float(pt.get("coverage", 0.60)) * 0.3
	pt["satisfaction"] = clampf(float(pt.get("satisfaction", 0.55)) * 0.99 + sat_target * 0.01, 0.10, 0.97)
	if float(pt.get("fleet_age", 7.0)) > 11.0 and Deterministic.chance(0.005):
		events.append({"type": "aging_fleet", "message": "فرسودگی ناوگان اتوبوسرانی - خرابی‌های پی‌درپی و نارضایتی مسافران", "age": pt["fleet_age"]})
	if float(pt.get("affordability", 0.70)) < 0.30 and Deterministic.chance(0.005):
		events.append({"type": "fare_protest", "message": "گرانی کرایه حمل‌ونقل عمومی - تجمع اعتراضی کارگران"})
	state["public_transport"] = pt
