...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	var _sys_extra = state.get("transport_roads", {})
	...
	var _sys_q = 0.60
	if state.has("transport_roads") and state["transport_roads"] is Dictionary:
		_sys_q = float(state["transport_roads"].get("quality",0.60) if state["transport_roads"].has("quality") else state["transport_roads"].get("efficiency",0.60) if state["transport_roads"].has("efficiency") else 0.60)
	_sys_q = clamp(_sys_q*0.96 + _trust*0.02 + (1.0-_corruption)*0.02 + _happiness*0.01 + Deterministic.next_range(-0.001,0.001), 0.05, 0.98)
	if state.has("transport_roads") and state["transport_roads"] is Dictionary:
		state["transport_roads"]["quality"] = _sys_q

	# نابرابری و نارضایتی
	if _gini > 0.42 and Deterministic.chance(0.007):
		events.append({"type":"inequality_transport_roads_deep","gini": _gini, "message":"نابرابری اثر بر transport_roads - شکاف طبقاتی"})

	# فناوری دوگانه (نظامی-غیرنظامی)
	if _digital > 0.65 and Deterministic.chance(0.006):
		events.append({"type":"dual_use_tech_transport_roads","digital": _digital, "message":"فناوری دوگانه در transport_roads - کاربرد نظامی و غیرنظامی"})
	...
	if _climate_resilience < 0.35 and Deterministic.chance(0.005):
		events.append({"type":"climate_vulnerability_transport_roads","resilience": _climate_resilience, "message":"آسیب‌پذیری اقلیمی transport_roads"})
	...
	if _social_capital < 0.40 and Deterministic.chance(0.006):
		events.append({"type":"low_social_capital_transport_roads","capital": _social_capital, "message":"سرمایه اجتماعی پایین در transport_roads"})

	# اثر تورمی بر هزینه نگهداری
	_inflation = float(_extra_econ.get("inflation",0.08))
	if state.has("transport_roads") and state["transport_roads"] is Dictionary and state["transport_roads"].has("maintenance_cost"):
		state["transport_roads"]["maintenance_cost"] = float(state["transport_roads"]["maintenance_cost"]) * (1.0 + _inflation*0.5/365.0)
