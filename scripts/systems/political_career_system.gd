...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	var _sys_extra = state.get("political_career", {})
	...
	var _sys_q = 0.60
	if state.has("political_career") and state["political_career"] is Dictionary:
		_sys_q = float(state["political_career"].get("quality",0.60) if state["political_career"].has("quality") else state["political_career"].get("efficiency",0.60) if state["political_career"].has("efficiency") else 0.60)
	_sys_q = clamp(_sys_q*0.96 + _trust*0.02 + (1.0-_corruption)*0.02 + _happiness*0.01 + Deterministic.next_range(-0.001,0.001), 0.05, 0.98)
	if state.has("political_career") and state["political_career"] is Dictionary:
		state["political_career"]["quality"] = _sys_q

	# نابرابری و نارضایتی
	if _gini > 0.42 and Deterministic.chance(0.007):
		events.append({"type":"inequality_political_career_deep","gini": _gini, "message":"نابرابری اثر بر political_career - شکاف طبقاتی"})

	# فناوری دوگانه (نظامی-غیرنظامی)
	if _digital > 0.65 and Deterministic.chance(0.006):
		events.append({"type":"dual_use_tech_political_career","digital": _digital, "message":"فناوری دوگانه در political_career - کاربرد نظامی و غیرنظامی"})
	...
	if _climate_resilience < 0.35 and Deterministic.chance(0.005):
		events.append({"type":"climate_vulnerability_political_career","resilience": _climate_resilience, "message":"آسیب‌پذیری اقلیمی political_career"})
	...
	if _social_capital < 0.40 and Deterministic.chance(0.006):
		events.append({"type":"low_social_capital_political_career","capital": _social_capital, "message":"سرمایه اجتماعی پایین در political_career"})

	# اثر تورمی بر هزینه نگهداری
	_inflation = float(_extra_econ.get("inflation",0.08))
	if state.has("political_career") and state["political_career"] is Dictionary and state["political_career"].has("maintenance_cost"):
		state["political_career"]["maintenance_cost"] = float(state["political_career"]["maintenance_cost"]) * (1.0 + _inflation*0.5/365.0)
