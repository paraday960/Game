...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# ── لایه واقع‌گرایانه اختصاصی محیط‌زیست (جایگزین قالب خودکار تکراری) — بخش ۳.۲۴ ──
	# تغییر اقلیم «ذخیره انباشته» است: انتشار منهای جذب (جنگل + انرژی پاک)؛ معکوس نمی‌شود، فقط کند می‌شود
	var emission_e: float = float(env.get("carbon_emission", 0.50))
	var absorb_e: float = float(env.get("forest_coverage", 0.20)) * 0.4 + float(env.get("green_energy_share", 0.20)) * 0.35
	env["climate_change"] = clampf(float(env.get("climate_change", 0.50)) + (emission_e - absorb_e) * 0.00004, 0.05, 0.98)
	# آلودگی هوا از انتشار صنعتی؛ جاروی طبیعی جنگل
	var air_target: float = clampf(0.25 + float(env.get("carbon_emission", 0.50)) * 0.5 - float(env.get("forest_coverage", 0.20)) * 0.15, 0.05, 0.90)
	env["pollution"] = clampf(float(env.get("pollution", 0.45)) * 0.998 + air_target * 0.002, 0.05, 0.95)
	if float(env.get("climate_change", 0.50)) > 0.75 and Deterministic.chance(0.005):
		events.append({"type": "heatwave", "message": "موج گرمای شدید - فشار بر شبکه برق و سلامت سالمندان", "climate": env["climate_change"]})
	if float(env.get("soil_quality", 0.60)) < 0.35 and Deterministic.chance(0.004):
		events.append({"type": "dust_storm", "message": "رگبار گرد و غبار - فرسایش خاک کانون‌های گرد و غبار را فعال کرد", "soil": env["soil_quality"]})
	state["environment"] = env
