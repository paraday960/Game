...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	var _sys_extra = state.get("foreign_affairs", {})
	...
	var _sys_q = 0.60
	if state.has("foreign_affairs") and state["foreign_affairs"] is Dictionary:
		_sys_q = float(state["foreign_affairs"].get("quality",0.60) if state["foreign_affairs"].has("quality") else state["foreign_affairs"].get("efficiency",0.60) if state["foreign_affairs"].has("efficiency") else 0.60)
	_sys_q = clamp(_sys_q*0.96 + _trust*0.02 + (1.0-_corruption)*0.02 + _happiness*0.01 + Deterministic.next_range(-0.001,0.001), 0.05, 0.98)
	if state.has("foreign_affairs") and state["foreign_affairs"] is Dictionary:
		state["foreign_affairs"]["quality"] = _sys_q

	# نابرابری و نارضایتی
	if _gini > 0.42 and Deterministic.chance(0.007):
		events.append({"type":"inequality_foreign_affairs_deep","gini": _gini, "message":"نابرابری اثر بر foreign_affairs - شکاف طبقاتی"})

	# فناوری دوگانه (نظامی-غیرنظامی)
	if _digital > 0.65 and Deterministic.chance(0.006):
		events.append({"type":"dual_use_tech_foreign_affairs","digital": _digital, "message":"فناوری دوگانه در foreign_affairs - کاربرد نظامی و غیرنظامی"})
	...
	if _climate_resilience < 0.35 and Deterministic.chance(0.005):
		events.append({"type":"climate_vulnerability_foreign_affairs","resilience": _climate_resilience, "message":"آسیب‌پذیری اقلیمی foreign_affairs"})
	...
	if _social_capital < 0.40 and Deterministic.chance(0.006):
		events.append({"type":"low_social_capital_foreign_affairs","capital": _social_capital, "message":"سرمایه اجتماعی پایین در foreign_affairs"})

	# اثر تورمی بر هزینه نگهداری
	_inflation = float(_extra_econ.get("inflation",0.08))
	if state.has("foreign_affairs") and state["foreign_affairs"] is Dictionary and state["foreign_affairs"].has("maintenance_cost"):
		state["foreign_affairs"]["maintenance_cost"] = float(state["foreign_affairs"]["maintenance_cost"]) * (1.0 + _inflation*0.5/365.0)
