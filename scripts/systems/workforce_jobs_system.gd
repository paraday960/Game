...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	var _sys_extra = state.get("workforce_jobs", {})
	...
	var _sys_q = 0.60
	if state.has("workforce_jobs") and state["workforce_jobs"] is Dictionary:
		_sys_q = float(state["workforce_jobs"].get("quality",0.60) if state["workforce_jobs"].has("quality") else state["workforce_jobs"].get("efficiency",0.60) if state["workforce_jobs"].has("efficiency") else 0.60)
	_sys_q = clamp(_sys_q*0.96 + _trust*0.02 + (1.0-_corruption)*0.02 + _happiness*0.01 + Deterministic.next_range(-0.001,0.001), 0.05, 0.98)
	if state.has("workforce_jobs") and state["workforce_jobs"] is Dictionary:
		state["workforce_jobs"]["quality"] = _sys_q

	# نابرابری و نارضایتی
	if _gini > 0.42 and Deterministic.chance(0.007):
		events.append({"type":"inequality_workforce_jobs_deep","gini": _gini, "message":"نابرابری اثر بر workforce_jobs - شکاف طبقاتی"})

	# فناوری دوگانه (نظامی-غیرنظامی)
	if _digital > 0.65 and Deterministic.chance(0.006):
		events.append({"type":"dual_use_tech_workforce_jobs","digital": _digital, "message":"فناوری دوگانه در workforce_jobs - کاربرد نظامی و غیرنظامی"})
	...
	if _climate_resilience < 0.35 and Deterministic.chance(0.005):
		events.append({"type":"climate_vulnerability_workforce_jobs","resilience": _climate_resilience, "message":"آسیب‌پذیری اقلیمی workforce_jobs"})
	...
	if _social_capital < 0.40 and Deterministic.chance(0.006):
		events.append({"type":"low_social_capital_workforce_jobs","capital": _social_capital, "message":"سرمایه اجتماعی پایین در workforce_jobs"})

	# اثر تورمی بر هزینه نگهداری
	_inflation = float(_extra_econ.get("inflation",0.08))
	if state.has("workforce_jobs") and state["workforce_jobs"] is Dictionary and state["workforce_jobs"].has("maintenance_cost"):
		state["workforce_jobs"]["maintenance_cost"] = float(state["workforce_jobs"]["maintenance_cost"]) * (1.0 + _inflation*0.5/365.0)
