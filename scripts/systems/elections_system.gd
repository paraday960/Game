...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# ── لایه واقع‌گرایانه اختصاصی انتخابات (جایگزین قالب خودکار تکراری) — بخش ۳.۶۵ ──
	# مشارکت رأی از اعتماد و شادی؛ سرخوردگی مشارکت را فرومی‌ریزد
	var turnout_target: float = clampf(0.30 + float(politics.get("trust", 0.55)) * 0.4 + float(pop.get("happiness", 0.60)) * 0.2, 0.10, 0.95)
	elections["participation"] = clampf(float(elections.get("participation", 0.60)) * 0.998 + turnout_target * 0.002, 0.05, 0.95)
	# شفافیت با فساد پایین و انسجام بالا
	var trans_target: float = clampf(0.70 - float(politics.get("corruption", 0.30)) * 0.6 + float(culture.get("cohesion", 0.65)) * 0.15, 0.05, 0.98)
	elections["transparency"] = clampf(float(elections.get("transparency", 0.55)) * 0.997 + trans_target * 0.003, 0.05, 0.98)
	if float(elections.get("participation", 0.60)) < 0.35 and Deterministic.chance(0.004):
		events.append({"type": "low_turnout", "message": "مشارکت سیاسی رو به فروریزش - سرخوردگی عمومی از صندوق رأی", "participation": elections["participation"]})
	state["elections"] = elections
