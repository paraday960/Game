...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# اثر ثبات بر کارآمدی
	var _efficiency = 0.5
	if state.get("human_states",{}).has("efficiency"):
		_efficiency = float(state["human_states"].get("efficiency",0.60))
	elif state.get("human_states",{}).has("quality"):
		_efficiency = float(state["human_states"].get("quality",0.60))

	_efficiency = clamp(_efficiency*0.97 + _stability*0.02 + _trust*0.01 - _corruption*0.01 + Deterministic.next_range(-0.002,0.002), 0.05, 0.98)
	if state.has("human_states") and state["human_states"] is Dictionary:
		state["human_states"]["efficiency"] = _efficiency
		state["human_states"]["quality"] = clamp(float(state["human_states"].get("quality",_efficiency))*0.98 + _efficiency*0.02, 0.05, 0.98)

	# اثر رشد و تورم بر بودجه داخلی سیستم
	var _sys_budget_share = float(_econ_extra.get("budget_allocations",{}).get("زیرساخت",0.15))
	var _maintenance_need = float(state.get("human_states",{}).get("quality",0.60) if state.has("human_states") else 0.60) * 0.02 * float(_econ_extra.get("gdp",500e9)) * 0.008
	var _actual_budget = float(_econ_extra.get("government_spending",95e9)) * _sys_budget_share
	var _budget_gap = _actual_budget - _maintenance_need
	if _budget_gap < 0 and Deterministic.chance(0.012):
		events.append({"type":"budget_gap_human_states","gap": _budget_gap, "message":"کسری بودجه نگهداری human_states - فرسودگی"})

	# اثر فناوری دیجیتال
	if _digital > 0.60 and Deterministic.chance(0.009):
		events.append({"type":"digital_boost_human_states","digital": _digital, "message":"جهش دیجیتال در human_states - اتوماسیون"})

	# اثر فساد
	if _corruption > 0.60 and Deterministic.chance(0.010):
		events.append({"type":"corruption_human_states_extra","corruption": _corruption, "message":"فساد در human_states - بازرسی"})

	# اثر نابرابری
	var _gini = float(_welfare_extra.get("gini",0.38))
	if _gini > 0.45 and Deterministic.chance(0.008):
		events.append({"type":"inequality_human_states","gini": _gini, "message":"نابرابری اثر بر human_states"})

	# اثر شادی و امید بر بهره‌وری
	var _productivity = float(state.get("human_states",{}).get("productivity",0.60) if state.has("human_states") else 0.60)
	_productivity = clamp(_productivity*0.98 + _happiness*0.01 + _growth*5.0*0.01 + _infra_q*0.01, 0.10, 0.95)
	if state.has("human_states") and state["human_states"] is Dictionary:
		state["human_states"]["productivity"] = _productivity

	# تاب‌آوری در برابر شوک
	var _resilience = float(state.get("human_states",{}).get("resilience",0.60) if state.has("human_states") else 0.60)
	_resilience = clamp(_resilience*0.96 + _stability*0.02 + _trust*0.01 + _cohesion*0.01, 0.10, 0.95)
	if state.has("human_states") and state["human_states"] is Dictionary:
		state["human_states"]["resilience"] = _resilience

	if _resilience < 0.32 and Deterministic.chance(0.011):
		events.append({"type":"low_resilience_human_states","resilience": _resilience, "message":"تاب‌آوری پایین human_states - شکننده در برابر شوک"})

	# اثر پوشش و دسترسی
	var _coverage = float(state.get("human_states",{}).get("coverage",0.70) if state.has("human_states") else 0.70)
	if _coverage < 0.50 and Deterministic.chance(0.010):
		events.append({"type":"coverage_human_states","coverage": _coverage, "message":"پوشش human_states پایین - دسترسی محدود"})
	...
	if state.has("human_states") and state["human_states"] is Dictionary:
		state["human_states"]["quality"] = _sys_q

	# نابرابری و نارضایتی
	if _gini > 0.42 and Deterministic.chance(0.007):
		events.append({"type":"inequality_human_states_deep","gini": _gini, "message":"نابرابری اثر بر human_states - شکاف طبقاتی"})
