...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	if float(gov.get("digital_government", 0.50)) > 0.65:
		pol_gb["corruption"] = clampf(float(pol_gb.get("corruption", 0.30)) - 0.00015, 0.02, 0.95)
		state["politics"] = pol_gb
	# میز خدمت یکپارچه: با دیجیتال‌شدن گسترش می‌یابد و زمان بروکراسی را می‌کاهد
	gov["one_stop_shops"] = maxi(int(float(gov.get("one_stop_shops", 120)) * (1.0 + float(gov.get("digital_government", 0.50)) * 0.0005)), 30)
	gov["paperwork_time_days"] = maxf(float(gov.get("paperwork_time_days", 12.0)) - float(gov.get("one_stop_shops", 120)) / 500.0 * 0.01, 1.0)
	# بار بروکراتیک بر کسب‌وکار: هر روز معطلی اداری هزینه است (بخش خصوصی می‌تواند بخواند)
	gov["bureaucracy_burden"] = clampf(float(gov.get("paperwork_time_days", 12.0)) / 30.0, 0.05, 1.0)
	# شهرداری‌ها باید متناسب سکونتگاه‌ها باشند — نه بیشتر نه کمتر (گرایش تدریجی به اندازه واقعی)
	var settlements_gb = state.get("settlements_detail", {})
	var towns_real = int(settlements_gb.get("cities_large", 50)) + int(settlements_gb.get("cities_medium", 200)) + int(settlements_gb.get("cities_small", 350)) + int(settlements_gb.get("towns", 400))
	gov["municipalities"] = maxi(int(float(gov.get("municipalities", 1200)) * 0.9995 + towns_real * 0.0005), 200)
	# رضایت شهروندی از خدمات دولتی = کارآمدی منهای زمان بروکراسی
	var sat_g = float(gov.get("efficiency", 0.60)) * 0.6 + (1.0 - float(gov.get("bureaucracy_burden", 0.4))) * 0.4
	gov["citizen_satisfaction"] = clampf(float(gov.get("citizen_satisfaction", 0.55)) * 0.99 + sat_g * 0.01, 0.10, 0.97)
	if float(gov.get("paperwork_time_days", 12.0)) > 20.0 and Deterministic.chance(0.005):
		events.append({"type": "bureaucracy_crisis", "message": "بیست روز معطلی برای یک مهر ساده - بروکراسی گلایه مردم شد", "days": gov["paperwork_time_days"]})
	if float(gov.get("digital_government", 0.50)) > 0.75 and Deterministic.chance(0.004):
		events.append({"type": "digital_gov_milestone", "message": "دولت الکترونیک بالغ شد - بیشتر خدمات اداری بدون مراجعه حضوری"})
	state["government_buildings"] = gov
