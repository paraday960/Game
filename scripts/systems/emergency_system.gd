...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# ── لایه واقع‌گرایانه اختصاصی مدیریت بحران (جایگزین قالب خودکار تکراری) — بخش ۳.۳۵ ──
	# آمادگی واقعی از تراکم پوشش: ایستگاه آتش‌نشانی، آمبولانس و تیم امداد نسبت به جمعیت
	var pop_e: float = float(state.get("population", {}).get("total", 85_000_000.0))
	var coverage_e: float = (float(emergency.get("fire_stations", 500.0)) / 800.0 + float(emergency.get("ambulances", 2000.0)) / maxf(pop_e / 50000.0, 1.0) + float(emergency.get("rescue_teams", 300.0)) / 500.0) / 3.0
	emergency["preparedness"] = clampf(float(emergency.get("preparedness", 0.50)) * 0.997 + clampf(coverage_e, 0.05, 1.0) * 0.003, 0.05, 0.95)
	# زمان پاسخ از کیفیت شبکه جاده‌ای
	var road_q_e: float = float(state.get("infrastructure", {}).get("road_quality", 0.55))
	emergency["response_time"] = clampf(float(emergency.get("response_time", 10.0)) * 0.998 + (14.0 - road_q_e * 8.0) * 0.002, 3.0, 30.0)
	if float(emergency.get("preparedness", 0.50)) < 0.25 and Deterministic.chance(0.004):
		events.append({"type": "emergency_unprepared", "message": "آمادگی بحران پایین - در سانحه بعدی تلفات سنگین خواهد بود", "preparedness": emergency["preparedness"]})
	state["emergency"] = emergency
