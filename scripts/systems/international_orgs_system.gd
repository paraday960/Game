...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	var _sys_extra = state.get("international_orgs", {})
	...
	var _sys_q = 0.60
	if state.has("international_orgs") and state["international_orgs"] is Dictionary:
		_sys_q = float(state["international_orgs"].get("quality",0.60) if state["international_orgs"].has("quality") else state["international_orgs"].get("efficiency",0.60) if state["international_orgs"].has("efficiency") else 0.60)
	_sys_q = clamp(_sys_q*0.96 + _trust*0.02 + (1.0-_corruption)*0.02 + _happiness*0.01 + Deterministic.next_range(-0.001,0.001), 0.05, 0.98)
	if state.has("international_orgs") and state["international_orgs"] is Dictionary:
		state["international_orgs"]["quality"] = _sys_q

	# نابرابری و نارضایتی
	if _gini > 0.42 and Deterministic.chance(0.007):
		events.append({"type":"inequality_international_orgs_deep","gini": _gini, "message":"نابرابری اثر بر international_orgs - شکاف طبقاتی"})

	# فناوری دوگانه (نظامی-غیرنظامی)
	if _digital > 0.65 and Deterministic.chance(0.006):
		events.append({"type":"dual_use_tech_international_orgs","digital": _digital, "message":"فناوری دوگانه در international_orgs - کاربرد نظامی و غیرنظامی"})
	...
	if _climate_resilience < 0.35 and Deterministic.chance(0.005):
		events.append({"type":"climate_vulnerability_international_orgs","resilience": _climate_resilience, "message":"آسیب‌پذیری اقلیمی international_orgs"})
	...
	if _social_capital < 0.40 and Deterministic.chance(0.006):
		events.append({"type":"low_social_capital_international_orgs","capital": _social_capital, "message":"سرمایه اجتماعی پایین در international_orgs"})

	# اثر تورمی بر هزینه نگهداری
	_inflation = float(_extra_econ.get("inflation",0.08))
	if state.has("international_orgs") and state["international_orgs"] is Dictionary and state["international_orgs"].has("maintenance_cost"):
		state["international_orgs"]["maintenance_cost"] = float(state["international_orgs"]["maintenance_cost"]) * (1.0 + _inflation*0.5/365.0)
