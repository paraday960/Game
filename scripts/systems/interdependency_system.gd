...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# ── لایه واقع‌گرایانه اختصاصی وابستگی متقابل (جایگزین قالب خودکار) — بخش ۳.۶۳ ──
	# گلوگاه لجستیک: شبکه راه/بندر ضعیف (راه‌ها دور ۱۲) زنجیره تأمین را می‌خفه — به فهرست گلوگاه‌ها اضافه می‌شود
	var logistics_i = float(state.get("transport_detail", {}).get("logistics_efficiency", 0.65))
	if logistics_i < 0.45:
		inter["bottlenecks"].append({"type": "لجستیک", "flow": logistics_i, "demand": 0.60, "severity": (0.60 - logistics_i)})
		inter["cascade_risk"] = clampf(float(inter.get("cascade_risk", 0.10)) + 0.06, 0.02, 0.92)
	# تنش آبی فصلی: تابستان برداشت آب بیشتر و رواناب کمتر است — جریان آب فصلی می‌شود
	var season_i = str(state.get("clock", {}).get("season", "بهار"))
	if season_i == "تابستان":
		inter["water_flow"] = clampf(float(inter.get("water_flow", 90.0)) * 0.9985, 10.0, 200.0)
