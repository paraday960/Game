...
func compute(state: Dictionary, tick: int) -> Dictionary:
	...
	# ── لایه واقع‌گرایانه اختصاصی زندان (جایگزین قالب خودکار تکراری) — بخش ۳.۶۷ ──
	# تکرار جرم: توانمندسازی/آموزش/اشتغال کاهش‌دهنده؛ ازدحام و شرایط بد افزایش‌دهنده
	var rehab: float = (float(prison.get("rehabilitation", 0.40)) + float(prison.get("education_prison", 0.35)) + float(prison.get("work_programs", 0.30))) / 3.0
	prison["recidivism"] = clampf(float(prison.get("recidivism", 0.35)) + (0.35 + float(prison.get("overcrowding", 0.80)) * 0.3 - rehab * 0.5 - float(prison.get("recidivism", 0.35))) * 0.003, 0.05, 0.90)
	# جمعیت زندان از نرخ جرم و ازدحام از نسبت جمعیت به ظرفیت
	var crime_p: float = float(state.get("judicial", {}).get("crime_rate", 50.0))
	prison["population"] = maxf(float(prison.get("population", 80000.0)) + (crime_p - 50.0) * 2.0, 5000.0)
	var cap_ratio: float = float(prison["population"]) / maxf(float(prison.get("capacity", 100000.0)), 1000.0)
	prison["overcrowding"] = clampf(float(prison.get("overcrowding", 0.80)) * 0.995 + cap_ratio * 0.005, 0.30, 1.60)
	if float(prison.get("overcrowding", 0.80)) > 1.2 and Deterministic.chance(0.004):
		events.append({"type": "prison_riot_risk", "message": "بحران ازدحام زندان‌ها - خطر شورش زندانیان", "overcrowding": prison["overcrowding"]})
	state["prison"] = prison
