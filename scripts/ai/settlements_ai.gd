extends BaseAI
# هوش تخصصی settlements - کیفیت سکونتگاه‌ها - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var sett = state.get("settlements_detail", {})
	...
	var metric_val = sett.get("housing_quality", 0.6)

	# شرط اضطراری بر اساس شاخص اصلی
	if sett.get("housing_quality", 0.6) < 0.6:
		var budget_cmd = build_budget_command(state, "زیرساخت")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if sett.get("sprawl",0.30) > 0.60:
			cmds.append(GameCommand.create_policy_change("urban_planning", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "settlements" in ["health","education","welfare","food_security","citizens"]:
	...
	if "settlements" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "settlements", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
