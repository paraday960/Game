extends BaseAI
# هوش تخصصی public_religious - دسترسی اماکن عمومی - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var pr = state.get("public_religious", {})
	...
	var metric_val = pr.get("access", 0.65)

	# شرط اضطراری بر اساس شاخص اصلی
	if pr.get("access", 0.65) < 0.65:
		var budget_cmd = build_budget_command(state, "اداره")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if pr.get("maintenance",0.60) < 0.40:
			cmds.append(GameCommand.create_policy_change("public_places_renovation", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "public_religious" in ["health","education","welfare","food_security","citizens"]:
	...
	if "public_religious" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "public_religious", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
