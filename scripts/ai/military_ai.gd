...
func decide(state: Dictionary, tick: int) -> Array:
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "military" in ["health","education","welfare","food_security","citizens"]:
	...
	if "military" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "military", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
