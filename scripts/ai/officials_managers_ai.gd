extends BaseAI
# هوش تخصصی officials_managers - شایستگی مدیران - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var off = state.get("officials", {})
	...
	var metric_val = off.get("competence", 0.6)

	# شرط اضطراری بر اساس شاخص اصلی
	if off.get("competence", 0.6) < 0.6:
		var budget_cmd = build_budget_command(state, "اداره")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if off.get("corruption",0.30) > 0.50:
			cmds.append(GameCommand.create_policy_change("anticorruption_drive", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "officials_managers" in ["health","education","welfare","food_security","citizens"]:
	...
	if "officials_managers" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "officials_managers", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
