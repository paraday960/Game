extends BaseAI
# هوش تخصصی health - کیفیت سلامت - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var hl = state.get("health", {})
	...
	var metric_val = hl.get("quality", 0.65)

	# شرط اضطراری بر اساس شاخص اصلی
	if hl.get("quality", 0.65) < 0.65:
		var budget_cmd = build_budget_command(state, "بهداشت")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if hl.get("coverage",0.75) < 0.60:
			cmds.append(GameCommand.create_policy_change("universal_health", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "health" in ["health","education","welfare","food_security","citizens"]:
	...
	if "health" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "health", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
