extends BaseAI
# هوش تخصصی politicians - اعتماد به سیاست‌مداران - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی
...
func decide(state: Dictionary, tick: int) -> Array:
	...
	var metric_val = pols.get("trust_politicians", 0.5)

	# شرط اضطراری بر اساس شاخص اصلی
	if pols.get("trust_politicians", 0.5) < 0.5:
		var budget_cmd = build_budget_command(state, "اداره")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if pols.get("polarization",0.40) > 0.65:
			cmds.append(GameCommand.create_policy_change("dialogue_forum", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "politicians" in ["health","education","welfare","food_security","citizens"]:
	...
	if "politicians" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "politicians", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
