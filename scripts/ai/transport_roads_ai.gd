extends BaseAI
# هوش تخصصی transport_roads - تراکم حمل‌ونقل - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var trd = state.get("transport_detail", {})
	...
	var metric_val = trd.get("traffic_congestion", 0.5)

	# شرط اضطراری بر اساس شاخص اصلی
	if trd.get("traffic_congestion", 0.5) > 0.5:
		var budget_cmd = build_budget_command(state, "زیرساخت")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if trd.get("road_quality",0.55) < 0.45:
			cmds.append(GameCommand.create_policy_change("road_maintenance", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "transport_roads" in ["health","education","welfare","food_security","citizens"]:
	...
	if "transport_roads" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "transport_roads", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
