extends BaseAI
# هوش تخصصی family - رفاه خانواده - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var fam = state.get("family", {})
	...
	var metric_val = fam.get("child_welfare", 0.65)

	# شرط اضطراری بر اساس شاخص اصلی
	if fam.get("child_welfare", 0.65) < 0.65:
		var budget_cmd = build_budget_command(state, "رفاه")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if fam.get("fertility",1.8) < 1.5:
			cmds.append(GameCommand.create_policy_change("child_support", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "family" in ["health","education","welfare","food_security","citizens"]:
	...
	if "family" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "family", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
