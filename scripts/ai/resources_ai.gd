extends BaseAI
# هوش تخصصی resources - خودکفایی منابع - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی
...
func decide(state: Dictionary, tick: int) -> Array:
	...
	var metric_val = res.get("self_sufficiency", 0.8)

	# شرط اضطراری بر اساس شاخص اصلی
	if res.get("self_sufficiency", 0.8) < 0.8:
		var budget_cmd = build_budget_command(state, "زیرساخت")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if res.get("energy_crisis", false):
			cmds.append(GameCommand.create_policy_change("energy_investment", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "resources" in ["health","education","welfare","food_security","citizens"]:
	...
	if "resources" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "resources", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
