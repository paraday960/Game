extends BaseAI
# هوش تخصصی public_employees - کارآمدی کارکنان عمومی - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var pe = state.get("public_employees", {})
	...
	var metric_val = pe.get("efficiency", 0.6)

	# شرط اضطراری بر اساس شاخص اصلی
	if pe.get("efficiency", 0.6) < 0.6:
		var budget_cmd = build_budget_command(state, "اداره")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if pe.get("satisfaction",0.55) < 0.35:
			cmds.append(GameCommand.create_policy_change("civil_service_reform", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "public_employees" in ["health","education","welfare","food_security","citizens"]:
	...
	if "public_employees" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "public_employees", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
