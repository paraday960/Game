extends BaseAI
# هوش تخصصی sports_youth - نشاط جوانان - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var sy = state.get("sports_youth", {})
	...
	var metric_val = sy.get("youth_happiness", 0.6)

	# شرط اضطراری بر اساس شاخص اصلی
	if sy.get("youth_happiness", 0.6) < 0.6:
		var budget_cmd = build_budget_command(state, "رفاه")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if sy.get("participation",0.40) < 0.30:
			cmds.append(GameCommand.create_policy_change("youth_sports_investment", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "sports_youth" in ["health","education","welfare","food_security","citizens"]:
	...
	if "sports_youth" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "sports_youth", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
