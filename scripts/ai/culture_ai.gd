extends BaseAI
# هوش تخصصی culture - انسجام فرهنگی - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var cult = state.get("culture", {})
	...
	var metric_val = cult.get("cohesion", 0.65)

	# شرط اضطراری بر اساس شاخص اصلی
	if cult.get("cohesion", 0.65) < 0.65:
		var budget_cmd = build_budget_command(state, "اداره")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if cult.get("media_freedom",0.5) < 0.40:
			cmds.append(GameCommand.create_policy_change("media_reform", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "culture" in ["health","education","welfare","food_security","citizens"]:
	...
	if "culture" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "culture", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
