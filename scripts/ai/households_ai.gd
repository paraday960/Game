extends BaseAI
# هوش تخصصی households - تاب‌آوری خانوار - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی
...
func decide(state: Dictionary, tick: int) -> Array:
	...
	var metric_val = hh.get("savings_rate", 0.15)

	# شرط اضطراری بر اساس شاخص اصلی
	if hh.get("savings_rate", 0.15) < 0.15:
		var budget_cmd = build_budget_command(state, "رفاه")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if hh.get("debt_ratio",0.20) > 0.50:
			cmds.append(GameCommand.create_policy_change("debt_relief", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "households" in ["health","education","welfare","food_security","citizens"]:
	...
	if "households" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "households", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
