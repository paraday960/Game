extends BaseAI
# هوش تخصصی stock_market - اعتماد بازار سرمایه - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var sm = state.get("stock_market", {})
	...
	var metric_val = sm.get("investor_confidence", 0.6)

	# شرط اضطراری بر اساس شاخص اصلی
	if sm.get("investor_confidence", 0.6) < 0.6:
		var budget_cmd = build_budget_command(state, "ذخیره")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if sm.get("index",1000.0) < 800.0:
			cmds.append(GameCommand.create_policy_change("market_stabilization", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "stock_market" in ["health","education","welfare","food_security","citizens"]:
	...
	if "stock_market" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "stock_market", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
