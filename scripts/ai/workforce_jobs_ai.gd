extends BaseAI
# هوش تخصصی workforce_jobs - اشتغال - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var wf = state.get("workforce_detail", {})
	...
	var metric_val = wf.get("unemployed", 0.1)

	# شرط اضطراری بر اساس شاخص اصلی
	if wf.get("unemployed", 0.1) > 0.1:
		var budget_cmd = build_budget_command(state, "آموزش")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if wf.get("skill_mismatch",0.30) > 0.50:
			cmds.append(GameCommand.create_policy_change("vocational_training", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "workforce_jobs" in ["health","education","welfare","food_security","citizens"]:
	...
	if "workforce_jobs" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "workforce_jobs", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
