extends BaseAI
# هوش تخصصی physical - کمبود مسکن - تحلیل دترمینستیک، بودجه‌محور، اقدام چندسطحی

func decide(state: Dictionary, tick: int) -> Array:
	var phys = state.get("physical", {})
	...
	var metric_val = phys.get("housing_shortage", 0.2)

	# شرط اضطراری بر اساس شاخص اصلی
	if phys.get("housing_shortage", 0.2) > 0.2:
		var budget_cmd = build_budget_command(state, "زیرساخت")
		if budget_cmd != null:
			cmds.append(budget_cmd)
		if phys.get("housing_shortage",0.15) > 0.30:
			cmds.append(GameCommand.create_policy_change("housing_program", true))
	...
	if _roi < 0.35 and cmds.is_empty():
		...
		if "physical" in ["health","education","welfare","food_security","citizens"]:
	...
	if "physical" in ["military","security","intelligence","security_forces"]:
...
func get_risk_assessment(state: Dictionary) -> Dictionary:
	var d = diagnose(state)
	return {"system": "physical", "health": d.get("health",0.5), "urgency": d.get("urgency",0.0), "risk_level": "بحرانی" if d.get("urgency",0.0) > 0.65 else "پرخطر" if d.get("urgency",0.0) > 0.35 else "عادی"}
